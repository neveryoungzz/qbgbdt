...
cdef class Stack:
    ...
    cdef int push(self, SIZE_t* samples, SIZE_t n_node_samples, SIZE_t depth, SIZE_t parent,
                  bint is_left, double impurity,
                  SIZE_t n_constant_features, double* prob_weight,
                  DTYPE_t* up_prob_bounds, DTYPE_t* low_prob_bounds,
                  bint is_past_prob) nogil except -1:
        ...
        stack[top].n_node_samples = n_node_samples
        stack[top].depth = depth
        stack[top].parent = parent
        stack[top].is_left = is_left
        stack[top].impurity = impurity
        stack[top].n_constant_features = n_constant_features
        stack[top].prob_weight = prob_weight
        stack[top].up_prob_bounds = up_prob_bounds
        stack[top].low_prob_bounds = low_prob_bounds
        stack[top].is_past_prob = is_past_prob
...
cdef class Pred_Stack:
    ...
    def __cinit__(self, SIZE_t capacity):
        self.capacity = capacity
        self.top = 0
        self.predstack_ = <PredStackRecord*> malloc(capacity * sizeof(PredStackRecord))

    def __dealloc__(self):
        free(self.predstack_)
    ...
    cdef int push(self, Node* node, double prob_weight) nogil except -1:
        """Push a new element onto the stack.
        ...
        """
        cdef SIZE_t top = self.top
        cdef PredStackRecord* predstack = NULL

        # Resize if capacity not sufficient
        if top >= self.capacity:
            self.capacity *= 2
            # Since safe_realloc can raise MemoryError, use `except -1`
            safe_realloc(&self.predstack_, self.capacity)

        predstack = self.predstack_
        predstack[top].node = node
        predstack[top].prob_weight = prob_weight

        # Increment stack pointer
        self.top = top + 1
        return 0

    cdef int pop(self, PredStackRecord* res) nogil:
        """Remove the top element from the stack and copy to ``res``.
        ...
        """
        cdef SIZE_t top = self.top
        cdef PredStackRecord* predstack = self.predstack_

        if top <= 0:
            return -1

        res[0] = predstack[top - 1]
        self.top = top - 1

        return 0
    
cdef class Pred_Grad_Stack:
    """A LIFO data structure.
    ...
    def __cinit__(self, SIZE_t capacity):
        self.capacity = capacity
        self.top = 0
        self.predgradstack_ = <PredGradStackRecord*> malloc(capacity * sizeof(PredGradStackRecord))

    def __dealloc__(self):
        free(self.predgradstack_)
    ...
    cdef int push(self, Node* node, double prob_weight,
                  DTYPE_t up_bound, DTYPE_t low_bound) nogil except -1:
        """Push a new element onto the stack.
        ...
        """
        cdef SIZE_t top = self.top
        cdef PredGradStackRecord* predgradstack = NULL

        # Resize if capacity not sufficient
        if top >= self.capacity:
            self.capacity *= 2
            # Since safe_realloc can raise MemoryError, use `except -1`
            safe_realloc(&self.predgradstack_, self.capacity)

        predgradstack = self.predgradstack_
        predgradstack[top].node = node
        predgradstack[top].prob_weight = prob_weight
        predgradstack[top].up_bound = up_bound
        predgradstack[top].low_bound = low_bound

        # Increment stack pointer
        self.top = top + 1
        return 0

    cdef int pop(self, PredGradStackRecord* res) nogil:
        """Remove the top element from the stack and copy to ``res``.
        ...
        """
        cdef SIZE_t top = self.top
        cdef PredGradStackRecord* predgradstack = self.predgradstack_

        if top <= 0:
            return -1

        res[0] = predgradstack[top - 1]
        self.top = top - 1
