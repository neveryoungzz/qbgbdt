...
cdef struct StackRecord:
    ...
    double* prob_weight
    DTYPE_t* up_prob_bounds
    DTYPE_t* low_prob_bounds
    bint is_past_prob

cdef struct PredStackRecord:
    Node* node
    double prob_weight

cdef struct PredGradStackRecord:
    Node* node
    double prob_weight
    DTYPE_t up_bound
    DTYPE_t low_bound
...
cdef class Pred_Stack:
    cdef SIZE_t capacity
    cdef SIZE_t top
    cdef PredStackRecord* predstack_
...
cdef class Pred_Grad_Stack:
    cdef SIZE_t capacity
    cdef SIZE_t top
    cdef PredGradStackRecord* predgradstack_
    cdef int reset(self) nogil except -1
    cdef bint is_empty(self) nogil
